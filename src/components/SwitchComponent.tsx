...
// }) => {
//   const [isEnabled, setIsEnabled] = useState(value); // Set initial value from props
//   const translateX = useRef(new Animated.Value(value ? 28 : 3)).current; // Starting position of the knob based on value

//   // Function to toggle the switch with animation
//   const animateSwitch = (newState: boolean) => {
//     Animated.timing(translateX, {
//       toValue: newState ? 28 : 3, // Move to 0 or 28 based on state
//       duration: 300, // Smooth transition duration (150ms)
//       useNativeDriver: true,
//     }).start();
...
//     if (value !== isEnabled) {
//       setIsEnabled(value);
//       translateX.setValue(value ? 28 : 3); // Update knob position when value prop changes
//     }
//   }, [value, translateX, isEnabled]);
...
const SwitchComponent: React.FC<Props> = ({
  ...
  const animateSwitch = (newState: boolean) => {
    Animated.timing(translateX, {
      toValue: newState ? 28 : 3, // Move to 3 or 28 based on state
      duration: 1500, // Smooth transition duration (400ms)
      easing: Easing.inOut(Easing.ease), // Use ease-in-out easing for smoother effect
      useNativeDriver: true,
    }).start();
